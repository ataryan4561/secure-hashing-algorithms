...
typedef struct {
    ...
    XMain_fn_uio_map maps[ MAX_UIO_MAPS ];
} XMain_fn_uio_info;

/***************** Variable Definitions **************************************/
static XMain_fn_uio_info uio_info;
...
static int uio_info_read_name(XMain_fn_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMain_fn_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMain_fn_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMain_fn_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMain_fn_Initialize(XMain_fn *InstancePtr, const char* InstanceName) {
	XMain_fn_uio_info *InfoPtr = &uio_info;
